...
`,
    ...
    ja: {
        ...
        tree: 'ツリー', 
        typography: 'タイポグラフィ',
        // Descriptions - Japanese content with the specific 問題改善プロセス text
        overviewDescription: `
# COSMOS農業デザインシステム

これは、スクリーンショットとインターフェースの問題に関する具体的なフィードバックに基づいて、COSMOSのこのデザインシステムを開発する最初のステップです。これにより現在の問題についての貴重な洞察を得ることができましたが、同時に限られた情報で作業していることも意味します。

## 問題改善プロセス

最善の作業を行うために、実際のプラットフォームを使用する必要があります。ワークフローをクリックして、さまざまなユーザーシナリオをテストし、実際の条件下でシステムがどのように動作するかを確認する必要があります。スクリーンショットはレイアウトの問題を示すことができますが、データの読み込み方法、エラーの処理方法、ピーク使用時のシステムのパフォーマンスを明らかにすることはできません。

この提案は、私たちが見て予想できる特定の問題に対処しますが、これは出発点に過ぎません。ライブプラットフォームと対話できるようになれば、静的な画像では見えない改善の機会を追加で発見するでしょう。

農業業界には独特の要求があります - 季節的ワークフロー、コンプライアンス要件、天候依存性、そして野外作業。これらの複雑さは、インターフェースがどのように適応し応答すべきかを真に理解するために、実践的なテストを必要とします。

ここで提示しているのは、明らかな問題を修正し、堅実な基盤を確立することです。しかし、真のデザインシステムの作業は、ライブプラットフォームで実際のユーザーが実際のタスクを完了するのを観察することから始まります。

## 私たちが変更したこと

- **テキストを読みやすく国際的にフレンドリーに：** 読みやすさを失うことなく、さまざまな言語でレイアウトをフレンドリーにするフォントサイズを標準化しました。これにより、特に他の言語でテキストが長くなる場合、すべての人がより良く読むことができます。また、入力フィールドを柔軟にして、より多くのスペースを必要とする言語に翻訳する際に壊れないようにしました。

- **テーブルをスマートカードに置き換え：** 最大の変更は、固定テーブルから柔軟なカードへの切り替えでした。テーブルは横スクロールを強制し、どのデバイスでも苛立たしく、電話では不可能でした。カードは自然に流れ、最初に最も重要な情報を表示し、必要に応じて詳細を利用できます。

- **複雑な情報の表示/非表示を追加：** すべてのユーザーが常にすべての詳細を必要とするわけではありません。製品名や数量などの必須情報は表示されたままです。価格設定、仕様、コンプライアンスの詳細はクリックすると展開されます。これにより画面がきれいに保たれ、パワーユーザーはすべてにアクセスできます。

## 次のステップ

- **プラットフォームアクセスと実ユーザーテスト：** 最初の優先事項は、ライブプラットフォームへの実践的なアクセスを取得することです。実際のワークフローをテストし、データの読み込み方法を観察し、完全なユーザージャーニーを理解する必要があります。

- **技術的な詳細調査：** プラットフォームアクセスにより、静的デザインでは見えないパフォーマンスのボトルネック、統合ポイント、データフローの問題を評価できます。

このロードマップは、私たちが実際に何に取り組んでいるかを理解することから始まり、あらゆる条件と文脈でユーザーにサービスを提供する包括的な農業プラットフォームの構築に向けて進みます。
`,
        liveDemo: 'ライブデモ',
        editHint: 'マークダウンを使用して内容を編集できます',
        launchForm: 'フォームを起動',
        launchInventory: '在庫を起動',
        // Form and other translations...
        farmInfo: '農場情報',
        orderNumber: '注文番号',
        deliveryPurpose: '配送目的',
        deliveryDate: '配送日',
        priorityLevel: '優先レベル',
        farmLocation: '農場の場所',
        autoGenerated: '保存時に自動生成',
        pleaseSelect: '選択してください',
        selectDate: 'mm/dd/yy',
        selectPurpose: '配送目的を選択してください',
        selectFarm: '農場の場所を選択...',
        expandInstructions: '各項目の展開ボタンをクリックして価格設定と備考にアクセス',
        expandAll: 'すべて展開',
        collapseAll: 'すべて折りたたむ',
        agriculturalProduct: '農産物',
        ...
        complete: '完了',
        seasonal: '季節性',
        // Inventory and other component translations
        high: '高',
        low: '低',
        normal: '通常',
        collected: '回収済み',
